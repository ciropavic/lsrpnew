...
export default Vue.extend({
 ...
 getbytes(e : string)
 {
    ...
    {
      document.getElementsByTagName('textarea')[0].value.replace('ㅅ','');
       if(!_iskey)
      // {this.getbytes(document.getElementsByTagName('textarea')[0].value[0])
      {
       document.getElementsByTagName('textarea')[0].value.replace('ㅅ','');
      }
      ...
      setTimeout(function(){
        oldVar = currentVar;
                ...
                e.key == '{' || e.key == '[' || e.key == ']' || e.key == '}' || e.key == '|' || e.key == '\\' ||  e.key == '/' || e.key == '.' || e.key == ',' || e.key == '?' )
            {
              ...
              || endtext == '[' || endtext == ']' || endtext == '{' || endtext == '}' || endtext == '\\' || endtext == '|' || endtext == '?')
              {
              }
              else
              {
    ...
    keyDown(e: KeyboardEvent) {
      ...
      // // {this.getbytes(document.getElementsByTagName('textarea')[0].value[0])
      // {
      //   if(document.getElementsByTagName('textarea')[0].value.charAt(0) == 'ㅅ')
      //   {
      //     var currentVar = document.getElementsByTagName('textarea')[0].value.substr(1);
      //     document.getElementsByTagName('textarea')[0].value = currentVar;
      //   }
      //   _iskey = true;
