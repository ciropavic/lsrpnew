...
window.addEventListener('message', function(event) {
	switch (event.data.action) {
			...
			if (selectedWindow == "none") {
				...
				$('#menu').html(`
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						<br>
						<div style="font-size: 30px; color: #fff;">잠시만 기다려 주세요.</div>
						<div style="font-size: 30px; color: #fff;">데이터를 로드하고 있습니다.</div>
					</div>
				`);
				$("#menu").fadeIn();
				$(".main_card").fadeIn();
				...
				setTimeout(function(){
					...
					$('#menu').html(`
						<div class="col-md-2 d-flex flex-column" style="border-right: 1px solid rgba(62, 63, 75); width: 177px;">
							...
							<hr>
							<span class="sidebar-title">개인 맞춤형 메뉴</span>
							<div id="sidebar"></div>
							<p class="sidebar-item mt-auto logout"><i class="fas fa-sign-out-alt"></i></i> <span class="ms-1">나가기</span></p>
						</div>
						<div class="col-md-10" style="width: 82%; display: flex; flex-direction: column;">
							<div class="d-flex justify-content-between align-items-center">
								<span class="selected-page" style="margin-left: 15px;"><span id="page-title">메인</span></span>
								<div>
									...
									<div style="font-size: 14px; margin-top: -12px; font-weight: 500; position: absolute; right: 7.2%;">
										<span>지갑: <span id="wallet_money"></span> USD</span>
									</div>
								</div>
							</div>
							<hr>
		...
		case 'transactions_page':
			...
			$('#page-title').html('거래내역');

			if (event.data.isInSociety){
				society = `<span class="sidebar-title mt-5">팩션 맞춤형 메뉴</span>
						   <p class="sidebar-item mt-2" id="society_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
						   <p class="sidebar-item" id="society_transactions" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>`;
			} else {
				society = '';
			}

			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
				<p class="sidebar-item selected" id="transactions_page" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>
				<p class="sidebar-item mt-2" id="settings_page" style="margin-bottom: 12px;"><i class="fas fa-cog"></i> <span class="ms-1">계좌설정</span></p>
				${society}
			`);
			...
			for(var i = 0; i < num; i++) {
				...
				if (db.type == 'transfer' && db.receiver_identifier == event.data.identifier) {
					...
					data = `<td class="align-middle" style="font-weight: 500;">
								입금 <span style="color: #1f5eff; font-weight: 600;">${db.sender_name}</span>
							</td>`;
					amount = `<td class="align-middle" style="font-weight: 500;"><span style="color: #1E90FF; float: right;">+ ${db.value.toLocaleString()} USD</span> </td>`;
				// Sent
					...
					data = `<td class="align-middle" style="font-weight: 500;">
								송금 <span style="color: #1f5eff; font-weight: 600;">${db.receiver_name}</span>
							</td>`;
					amount = `<td class="align-middle" style="font-weight: 500;"><span style="color: #FF0000; float: right;"> - ${db.value.toLocaleString()} USD</span> </td>`;
				// Deposited
				} else if (db.type == 'deposit') {
					icon = '<td class="align-middle"><span style="background-color: #1d1e24; padding: 10px 15px 10px 15px; border-radius: 10px;"><i class="bi bi-download"></i></span></td>';
					data = `<td class="align-middle" style="font-weight: 500;">
								입금 <span style="color: #1f5eff; font-weight: 600;">${db.receiver_name}</span>
							</td>`;
					amount = `<td class="align-middle" style="font-weight: 500;"><span style="color: #1E90FF; float: right;">+ ${db.value.toLocaleString()} USD</span> </td>`;
				// Withdrawn
					...
					icon = '<td class="align-middle"><span style="background-color: #1d1e24; padding: 10px 15px 10px 15px; border-radius: 10px;"><i class="bi bi-upload"></i></span></td>';
					data = `<td class="align-middle" style="font-weight: 500;">
								출금 <span style="color: #1f5eff; font-weight: 600;">${db.sender_name}</span>
							</td>`;
					amount = `<td class="align-middle" style="font-weight: 500;"><span style="color: #FF0000; float: right;"> - ${db.value.toLocaleString()} USD</span> </td>`;
				}
					...
					<div class="col col-md-3">
						<div class="card" style="background-color: #1f5eff; border-radius: 10px; color: #fff;">
						  <div class="card-body text-center">
						    <h6 class="card-title" style="font-weight: 600; font-size: 20px;">거래내역</h6>
						    <p class="card-text" id="totalInvoices" style="font-size: 20px;">${numOfTransactions} 건</p>
						  </div>
						</div>
					</div>
					<div class="col col-md-3">
						<div class="card" style="background-color: #1f5eff; border-radius: 10px; color: #fff;">
						  <div class="card-body text-center">
						    <h6 class="card-title" style="font-weight: 600; font-size: 20px;">입금</h6>
						    <p class="card-text" id="totalIncome" style="font-size: 20px;">${event.data.graph_values[7].toLocaleString()} USD</p>
						  </div>
						</div>
					</div>
					<div class="col col-md-3">
						<div class="card" style="background-color: #1f5eff; border-radius: 10px; color: #fff;">
						  <div class="card-body text-center">
						    <h6 class="card-title" style="font-weight: 600; font-size: 20px;">출금</h6>
						    <p class="card-text" id="unpaidInvoices" style="font-size: 20px;">${event.data.graph_values[8].toLocaleString()} USD</p>
						  </div>
						</div>
					</div>
					<div class="col col-md-3">
						<div class="card" style="background-color: #1f5eff; border-radius: 10px; color: #fff;">
						  <div class="card-body text-center">
						    <h6 class="card-title" style="font-weight: 600; font-size: 20px;">손익</h6>
						    <p class="card-text" id="awaitedIncome" style="font-size: 20px;">${event.data.graph_values[9].toLocaleString()} USD</p>
						  </div>
						</div>
					</div>
				</div>
					...
					<tbody id="transactionsData">
						<tr>
							...
							<td class="align-middle" style="font-weight: 500; font-size: 16px;">
								출금 <span style="color: #1f5eff; font-weight: 600;">Arthur James</span>
							</td>
						...
						<tr>
							...
							<td class="align-middle" style="font-weight: 500; font-size: 16px;">
								출금 <span style="color: #1f5eff; font-weight: 600;">Bank Account</span>
							</td>
							<td class="align-middle" style="font-weight: 500;">28 Aug 04:43</td>
		...
		case 'society_transactions':
			...
			$('#page-title').html('거래내역 ['+event.data.societyInfo.society_name+']');

			if (event.data.isInSociety){
				society = `<span class="sidebar-title mt-5">팩션 맞춤형 메뉴</span>
						   <p class="sidebar-item mt-2" id="society_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
						   <p class="sidebar-item selected" id="society_transactions" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>`;
			} else {
				society = '';
			}
			...
			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
				<p class="sidebar-item" id="transactions_page" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>
				<p class="sidebar-item mt-2" id="settings_page" style="margin-bottom: 12px;"><i class="fas fa-cog"></i> <span class="ms-1">계좌설정</span></p>
				${society}
			`);
...
function overview_page_function(event) {
	...
	$('#sidebar').html(`
		<p class="sidebar-item mt-2 selected" id="overview_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
		<p class="sidebar-item" id="transactions_page" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>
		<p class="sidebar-item mt-2" id="settings_page" style="margin-bottom: 12px;"><i class="fas fa-cog"></i> <span class="ms-1">계좌설정</span></p>
		${society}
	`);
		...
		<div class="col-md-7" style="border-right: 1px solid rgba(62, 63, 75);">
			<div class="card text-center" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75);">
				<div class="card-header" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
					<span style="color: #fff; font-size: 18px;"><i class="fas fa-chart-line"></i> 거래 추이</span>
				</div>
				...
				<div class="col-md-12">
					<div class="card h-100" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75); margin-top: 12px;">
						<div class="card-header text-center" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
							<span style="color: #fff; font-size: 18px;"><span><i class="fas fa-exchange-alt"></i> 최근 거래</span> <span class="badge bg-primary viewall-badge" style="position: absolute; font-size: 14px; right: 9px; top: 10px;" id="view_all_transactions"><i class="fas fa-eye"></i> 전체 보기</span></span>
						</div>
							...
							<table id="lastTransactionsTable">
								<tbody id="lastTransactionsData">
									...
									<tr>
										<td class="align-middle"><span style="background-color: #1d1e24; padding: 5px 10px 5px 10px; border-radius: 10px;"><i class="bi bi-upload"></i></span></td>
										<td class="align-middle" style="font-weight: 500; font-size: 16px;">
											입금 <span style="color: #1f5eff; font-weight: 600;">Arthur James</span>
										</td>
									...
									<tr>
										<td class="align-middle"><span style="background-color: #1d1e24; padding: 5px 10px 5px 10px; border-radius: 10px;"><i class="bi bi-download"></i></span></td>
										<td class="align-middle" style="font-weight: 500; font-size: 16px;">
											입금 <span style="color: #1f5eff; font-weight: 600;">Bank Account</span>
										</td>
		...
		<div class="col-md-5">
			<div class="col-md-12">
				<div class="card" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75);">
					<div class="card-header text-center" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
						<span style="color: #fff; font-size: 18px;"><i class="bi bi-credit-card-fill"></i> 내 계좌</span>
					</div>
					<div class="card-body" style="background-color: #1d1e24; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; font-size: 16px;">
						<div class="card creditcard-classic_card d-flex align-items-center" style="margin: 0 auto; margin-bottom: 16px;">
							<div class="card-body creditcard-classic_card-body">
								<span class="d-flex justify-content-between"><span><img src="visa_white.svg" style="width: 20%;"><span style="color: #fff; font-size: 12px; margin-left: 5px; font-weight: 500;">LS Bank Classic</span></span><span><i class="fas fa-wifi"></i></span></span>
						...
						<hr>
						<p class="card-text text-center" style="font-size: 20px;"><span style="color: #fff;">현재잔액:</span> <span id="playerBankMoney"></span> USD</p>
						<p class="card-text text-center" style="font-size: 20px;"><span style="color: #fff;">계좌번호:</span> <span id="playerIBAN">OK182716</span></p>
					</div>
				</div>
				<!--<br>
				<div class="card text-center" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75);">
					...
					<div class="card-body" style="background-color: #1d1e24; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; font-size: 16px;">
						<p class="card-text"><span style="color: #fff;">현재잔액:</span> 1.500.000 USD</p>
						<p class="card-text"><span style="color: #fff;">계좌번호:</span> OK182716</p>
					</div>
				</div>-->
				<div class="card text-center h-100" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75); margin-top: 12px;">
					<div class="card-header" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
						<span style="color: #fff; font-size: 18px;"><i class="fas fa-list-ul"></i> 바로가기</span>
					</div>
					<div class="card-body" style="background-color: #1d1e24; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
						<div class="d-flex justify-content-center">
							<button type="button" id="depositMoneyModal" class="btn btn-blue flex-grow-1" style="border-radius: 10px; flex-basis: 100%;" data-bs-toggle="modal" data-bs-target="#depositModal"><i class="bi bi-upload"></i> 입금</button>
						</div>
						<div class="d-flex justify-content-center" style="margin-top: 7px;">
							<button type="button" id="withdrawMoneyModal" class="btn btn-blue flex-grow-1" style="border-radius: 10px; flex-basis: 100%;" data-bs-toggle="modal" data-bs-target="#withdrawModal"><i class="bi bi-download"></i> 출금</button>
						</div>
						<div class="d-flex justify-content-center" style="margin-top: 7px;">
							<button type="button" id="transferMoneyModal" class="btn btn-blue flex-grow-1" style="border-radius: 10px; flex-basis: 100%;" data-bs-toggle="modal" data-bs-target="#transferModal"><i class="fas fa-exchange-alt"></i> 송금</button>
						</div>
					</div>
				</div>
			</div>
...
function society_page_function(event) {
	...
	$('#page-title').html('메인 ['+event.data.societyInfo.society_name+']');

	if (event.data.isInSociety){
		society = `<span class="sidebar-title mt-5">팩션 맞춤형 메뉴</span>
				   <p class="sidebar-item mt-2 selected" id="society_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
				   <p class="sidebar-item" id="society_transactions" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>`;
	} else {
		society = '';
	}
	...
	$('#page_info').html(`
		<div class="col-md-7" style="border-right: 1px solid rgba(62, 63, 75);">
			...
			<div class="row">
				<div class="col-md-12">
					<div class="card h-100" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75); margin-top: 12px;">
						<div class="card-header text-center" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
							<span style="color: #fff; font-size: 18px;"><span><i class="fas fa-exchange-alt"></i> 최근 거래</span> <span class="badge bg-primary viewall-badge" style="position: absolute; font-size: 14px; right: 9px; top: 10px;" id="view_all_transactions_society"><i class="fas fa-eye"></i> 전체 보기</span></span>
						</div>
						<div class="card-body" style="background-color: #1d1e24; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; padding: 0.6rem 1rem;">
							<table id="lastTransactionsTable">
								<tbody id="lastTransactionsData">
		...
		<div class="col-md-5">
			<div class="col-md-12">
				<div class="card" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75);">
					<div class="card-header text-center" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
						<span style="color: #fff; font-size: 18px;"><i class="bi bi-credit-card-fill"></i> 팩션 계좌</span>
					</div>
					<div class="card-body" style="background-color: #1d1e24; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; font-size: 16px;">
						<div class="card creditcard-classic_card d-flex align-items-center" style="margin: 0 auto; margin-bottom: 16px;">
							<div class="card-body creditcard-classic_card-body">
								<span class="d-flex justify-content-between"><span><img src="visa_white.svg" style="width: 20%;"><span style="color: #fff; font-size: 12px; margin-left: 5px; font-weight: 500;">LS Bank Society</span></span><span><i class="fas fa-wifi"></i></span></span>
								<div style="margin-top: 38%;">
									<div class="d-flex align-items-center">
										<span style="font-weight: 500; color: #fff; line-height: 1;">Status</span>
									</div>
...
function settings_page_function(event) {
	...
	$('#sidebar').html(`
		<p class="sidebar-item mt-2" id="overview_page" style="margin-bottom: 12px;"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">메인</span></p>
		<p class="sidebar-item" id="transactions_page" style="margin-bottom: 12px;"><i class="fas fa-exchange-alt"></i> <span class="ms-1">거래내역</span></p>
		<p class="sidebar-item mt-2 selected" id="settings_page" style="margin-bottom: 12px;"><i class="fas fa-cog"></i> <span class="ms-1">계좌설정</span></p>
		${society}
	`);
	...
	$('#page_info').html(`
		<div class="col-md-12 d-flex flex-grow-1 flex-column h-100" style="flex: 1 1 50%;">
			<div class="card d-flex flex-column flex-grow-1" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75);">
				<div class="card-header text-center" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
					<span style="color: #fff; font-size: 18px;">
						<span><i class="fas fa-address-card"></i> 계좌번호</span>
					</span>
				</div>
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100" style="background-color: #292a31; border-radius: 10px; color: #fff;">
								  <div class="card-body text-center">
								    <span style="font-size: 24px; color: #fff;">계좌번호 변경</span>
									<input type="text" maxlength="${event.data.ibanCharNum}" id="new_iban" class="form-control text-center" placeholder="계좌번호" onkeyup="checkIfEmptySettings()" style="margin-top: 12px; width: 100%;">
									<button type="button" id="change_iban" class="btn btn-blue" style="border-radius: 10px; flex-basis: 100%; margin-top: 12px; width: 100%;" disabled><i class="fas fa-paper-plane"></i> 변경</button>
								  </div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center" style="background-color: #1f5eff; border-radius: 10px; color: #fff;">
								  <div class="card-body text-center" style="font-size: 18px;">
								    <span>계좌번호 변경 비용은 5,000 USD입니다.</span>
								    <hr style="color: #fff;">
								    <span>계좌 위치는 "${event.data.ibanPrefix}"(으)로 자동 설정됩니다.</span>
								    <hr style="color: #fff;">
									<span>반드시 ${event.data.ibanCharNum}자리 숫자로 작성하셔야 합니다.</span>
								  </div>
								</div>
							</div>
						</div>
			...
			<div class="card d-flex flex-column flex-grow-1" style="background-color: transparent; border-radius: 10px; box-shadow: 0px 0px 5 0px rgba(0, 0, 0, 0.75); margin-top: 12px;">
				<div class="card-header text-center" style="background-color: #292a31; border-top-right-radius: 10px; border-top-left-radius: 10px;">
					<span style="color: #fff; font-size: 18px;">
						<span><i class="fas fa-key"></i> 비밀번호</span>
					</span>
				</div>
				<div class="card-body flex-grow-1" style="background-color: #1d1e24; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
					<div class="">
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100" style="background-color: #292a31; border-radius: 10px; color: #fff;">
								  <div class="card-body text-center">
								    <span style="font-size: 24px; color: #fff;">비밀번호 설정·변경</span>
									<input type="password" maxlength="4" id="new_pin" class="form-control text-center" placeholder="비밀번호" onkeyup="checkIfEmptySettings()" style="margin-top: 12px; width: 100%;">
									<button type="button" id="change_pin" class="btn btn-blue" style="border-radius: 10px; flex-basis: 100%; margin-top: 12px; width: 100%;" disabled><i class="fas fa-paper-plane"></i> 설정·변경</button>
								  </div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center" style="background-color: #1f5eff; border-radius: 10px; color: #fff;">
								  <div class="card-body text-center" style="font-size: 18px;">
								    <span>비밀번호 변경 비용은 1,000 USD입니다.</span>
								    <hr style="color: #fff;">
									<span>반드시 ${event.data.pinCharNum}자리 숫자로 작성하셔야 합니다.</span>
									<hr style="color: #fff;">
									<span>미설정 시 ATM 이용이 불가합니다.</span>
								  </div>
								</div>
							</div>
						</div>
	...
	setTimeout(function () {
		$('#menu').html(`
			<div class="d-flex justify-content-center flex-column align-items-center">
				...
				<br>
				<div style="font-size: 30px; color: #fff;">잠시만 기다려 주세요.</div>
				<div style="font-size: 30px; color: #fff;">데이터를 저장하고 있습니다.</div>
			</div>
		`);
		$("#menu").fadeIn();
