...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "은행", "은행에 "..amount.." USD를 입금했습니다.", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "그만한 현금을 소지하고 있지 않습니다.", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "은행", "은행에서 "..amount.." USD를 출금했습니다.", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "계좌에 그만한 예금이 없습니다.", 5000, 'error')
	end
end)
		...
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
				    	...
				    	TriggerClientEvent('okokNotify:Alert', xPlayers[i], "은행", xPlayer.getName().."님이 당신의 계좌로 "..amount.. " USD를 송금했습니다.", 5000, 'success')
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, source)
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', source, "은행", xTarget.getName().."님의 계좌로 "..amount.." USD를 송금했습니다.", 5000, 'success')
			elseif xTarget == nil then
				...
				MySQL.Async.insert('INSERT INTO okokBanking_transactions (receiver_identifier, receiver_name, sender_identifier, sender_name, date, value, type) VALUES (@receiver_identifier, @receiver_name, @sender_identifier, @sender_name, CURRENT_TIMESTAMP(), @value, @type)', {
					...
					['@receiver_name'] = tostring(result[1].firstname .. " " .. result[1].lastname),
				...
				tName = tostring(result[1].firstname .. " " .. result[1].lastname)

				TriggerClientEvent('okokBanking:updateTransactions', xPlayer.source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', xPlayer.source, "은행", tName.."님의 계좌로 "..amount.." USD를 송금했습니다. ", 5000, 'success')
			...
			TriggerClientEvent('okokNotify:Alert', source, "은행", "은행에 그만한 예금이 없습니다.", 5000, 'error')
		end
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "본인 계좌로 송금할 수 없습니다.", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerEvent('okokBanking:AddDepositTransactionToSociety', amount, source, society, xPlayer.getName())
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "은행", "팩션 ("..societyName..") 계좌에 "..amount.. " USD를 입금했습니다.", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "그만한 현금을 소지하고 있지 않습니다.", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', _source, "은행", "Someone is already withdrawing", 5000, 'error')
		else
			...
			xPlayer.addAccountMoney('money', amount)
			--xPlayer.addAccountMoney('bank', amount)
			TriggerEvent('okokBanking:AddWithdrawTransactionToSociety', amount, _source, society, xPlayer.getName())
			TriggerClientEvent('okokBanking:updateTransactionsSociety', _source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', _source, "은행", "팩션 ("..societyName..") 계좌에서 "..amount.. " USD를 출금했습니다.", 5000, 'success')
		end
	else
		TriggerClientEvent('okokNotify:Alert', _source, "은행", "팩션 ("..societyName..") 계좌에 그만한 예금이 없습니다.", 5000, 'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.getAccount('bank').money
		...
		if amount <= playerMoney then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "은행", "팩션 ("..societyName..") 계좌로 "..amount.." USD를 송금했습니다.", 5000, 'success')
		else
			TriggerClientEvent('okokNotify:Alert', source, "은행", "계좌에 그만한 예금이 없습니다.", 5000, 'error')
		end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "은행", "팩션 ("..societyNameTarget..") 계좌로 "..societyNameTarget.." USD를 송금했습니다.", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "계좌에 그만한 예금이 없습니다.", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:TransferMoneyToPlayerFromSociety", function(amount, ibanNumber, targetIdentifier, acc, targetName, society, societyName, societyMoney, toMyself)
	...
	if amount <= societyMoney then
		...
		if xTarget ~= nil then
			...
			if not toMyself then
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
			    		...
			    		TriggerClientEvent('okokBanking:updateTransactions', xPlayers[i], xTarget.getAccount('bank').money, xTarget.getMoney())
			    		TriggerClientEvent('okokNotify:Alert', xPlayers[i], "은행", "팩션 ("..societyName..") (이)가 당신의 계좌로 "..amount.." USD를 송금했습니다. ", 5000, 'success')
				    end
				end
			end
			...
			TriggerEvent('okokBanking:AddTransferTransactionFromSocietyToP', amount, society, societyName, targetIdentifier, tName.." ("..aName..")")
			TriggerClientEvent('okokBanking:updateTransactionsSociety', xPlayer.source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', xPlayer.source, "은행", xTarget.getName().."님의 계좌로 "..amount.." USD를 송금했습니다. ", 5000, 'success')
		elseif xTarget == nil then
			local playerAccount = json.decode(acc)
			playerAccount.bank = playerAccount.bank + amount
			playerAccount = json.encode(playerAccount)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Config.IBANChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "은행", "계좌번호가 "..iban.."(으)로 변경되었습니다.", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "계좌번호 변경을 위해선 "..Config.IBANChangeCost.."USD가 필요합니다.", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if Config.PINChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "은행", "비밀번호가 "..pin.."(으)로 변경되었습니다.", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "은행", "비밀번호 변경을 위해선 "..Config.PINChangeCost.."USD가 필요합니다.", 5000, 'error')
	end
end)