    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Removed everything from your cart!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("There's nothing in your cart!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            {label = '<span style="color:lightgreen; border-bottom: 1px solid lightgreen;">Pay</span>', value = "yes"},
            {label = 'Amount: <span style="color:green">$' .. payAmount ..'</span>'},
        }
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("You do not have enough money!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Added to cart: " .. data.current.value .. " " .. data.current["realLabel"] .. " ", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
