...
function menuVehicle(event) {
    ...
    $.post('https://eotix-vehicleshop/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://eotix-vehicleshop/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://eotix-vehicleshop/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('https://eotix-vehicleshop/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://eotix-vehicleshop/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://eotix-vehicleshop/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://eotix-vehicleshop/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://eotix-vehicleshop/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://eotix-vehicleshop/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://eotix-vehicleshop/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
