...
AddEventHandler("vrp_stores:buyItem",function(key,data)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local user_id = xPlayer.identifier
	local uselicence = true
	if data.license=='1' then
	local sql_licence = "SELECT * FROM user_licenses WHERE owner = @owner and type = 'weapon'";
	local query_licence = MySQL.Sync.fetchAll(sql_licence,{['@owner'] = user_id});
		if not query_licence[1] then
			uselicence = false
			else
		 uselicence = true
		end
...
if uselicence then
	...
	TriggerClientEvent('okokNotify:Alert', source, "시스템", "화기 라이센스가 없습니다.", 5000, 'error')
end
end)
