							...
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									...
									<div class="media-body">
										...
										<button onclick="buyItem('` + fkey + `','`+item.license+`')" class="btn btn-blue btn-darken-2 white deposit-money-btn col-6">Buy Item</button>
									</div>
									<div class="align-self-center">
...
function buyItem(item_id,license){
	var amount = $("#input-"+item_id).val();
	var item_id = item_id.replace("-x-", "|");
	post("buyItem",{item_id:item_id,amount:amount,license : license})
}
...
function post(name,data){
	$.post("http://esx_stores/"+name,JSON.stringify(data),function(datab){
		if (datab != "ok"){
			console.log(datab);
		}
	});
