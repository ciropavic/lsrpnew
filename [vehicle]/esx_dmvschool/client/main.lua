...
-- local StartTimer = true -- true시 진행 false시 멈춤
local Licenses          = {}
...
function OpenDMVSchoolMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dmvschool_actions', {
		...
		elseif data.current.value == 'drive_test' then
			...
			-- else
				-- ESX.ShowNotification("실기 테스트 탈락 시, 30분 후 재응시 가능합니다.")
			-- end
		end
...
RegisterNUICallback('close', function(data, cb)
	StopTheoryTest(true)
	Wait(1000)
	TriggerEvent('cui_character:kick')
	cb()
end)
...
RegisterNetEvent('esx_dmvschool:launchtest')
AddEventHandler('esx_dmvschool:launchtest', function()
	...
	while true do
		...
		if CurrentTest == 'drive' then
			...
			if DriveErrors >= Config.MaxErrors then
				...
				SetEntityCoords(PlayerPedId(), 215.52438354492, -1398.4116210938, 29.5935, false, false, false, true)
				-- StartTimer = false
				DriveErrors = 0
