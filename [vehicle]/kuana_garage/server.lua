...
RegisterServerEvent("kuana:deleteZeroCars")
AddEventHandler("kuana:deleteZeroCars",function(vehicle,plate)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	MySQL.Sync.execute("DELETE FROM `owned_vehicles` WHERE health = @health", { 
        ['@health'] = 0 
	})
	MySQL.Sync.execute("DELETE FROM `vehicle_keys` WHERE plate = @plate", { 
		['@plate'] = plate 
	})
	TriggerClientEvent("kuana:explode", xPlayer.source, vehicle)
...
AddEventHandler('garagem:save', function(plate, x, y, z, headings, heal)
	...
	local vehicules = getPlayerVehicles(iden)
	local state = true
	local h = headings
	local he = heal
	-- print(he)
...
ESX.RegisterServerCallback('kuana:checkcoordsall', function(source, cb, plate)
	...
	local zz = MySQL.Sync.fetchScalar("SELECT z FROM owned_vehicles WHERE plate = @plate", {['@plate'] = plate})        
	local hh = MySQL.Sync.fetchScalar("SELECT h FROM owned_vehicles WHERE plate = @plate", {['@plate'] = plate})
	local color1 = MySQL.Sync.fetchScalar("SELECT color1 FROM owned_vehicles WHERE plate = @plate", {['@plate'] = plate})
...
ESX.RegisterServerCallback('kuana:checkOwnerOfCar',function(source,cb,plate)
