...
Citizen.CreateThread(function()
	...
	mainMenu = NativeUI.CreateMenu(Config.servername, _U('mainmenu_subtitle'),1430,480)
	itemMenu = NativeUI.CreateMenu(Config.servername, _U('inventory_actions_subtitle'),1430,480)
	weaponItemMenu = NativeUI.CreateMenu(Config.servername, _U('loadout_actions_subtitle'),1430,480)
...
function AddMenuWalletMenu(menu)
	...
	walletmenu = _menuPool:AddSubMenu(menu, _U('wallet_title'),nil,true)
...
function AddMenuAdminMenu(menu, playerGroup)
	adminMenu = _menuPool:AddSubMenu(menu, _U('admin_title'),nil,true)
...
RegisterCommand("시동",function(a,b,c)

	if not IsPedSittingInAnyVehicle(plyPed) then
		exports['okokNotify']:Alert("시스템", "차량을 탑승한 상태가 아닙니다.", 5000, 'error')
		-- ESX.ShowNotification(_U('차량을 탑승한 상태가 아닙니다.'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		...
		ESX.TriggerServerCallback('KorioZ-havecarkey',function (result)
			if result == 1 then
				plyVehicle = GetVehiclePedIsIn(plyPed, false)
				if item == vehEngineItem then
					if GetIsVehicleEngineRunning(plyVehicle) then
						SetVehicleEngineOn(plyVehicle, false, false, true)
						SetVehicleUndriveable(plyVehicle, true)
						exports['okokNotify']:Alert("시스템", "차량 시동을 껐습니다.", 5000, 'success')
					elseif not GetIsVehicleEngineRunning(plyVehicle) then
						SetVehicleEngineOn(plyVehicle, true, false, true)
						SetVehicleUndriveable(plyVehicle, false)
						exports['okokNotify']:Alert("시스템", "차량 시동을 걸었습니다.", 5000, 'success')
					end
				end
			else 
				exports['okokNotify']:Alert("시스템", "해당 차량의 차키가 없습니다.", 5000, 'error')
			end
		end,plate)
...
RegisterCommand("본넷",function(a,b,c)
	if not IsPedSittingInAnyVehicle(plyPed) then
		ESX.ShowNotification(_U('no_vehicle'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		plyVehicle = GetVehiclePedIsIn(plyPed, false)
		if item == vehHoodItem then
			if not personalmenu.hoodDoorOpen then
				personalmenu.hoodDoorOpen = true
				SetVehicleDoorOpen(plyVehicle, 4, false, false)
			elseif personalmenu.hoodDoorOpen then
...
RegisterCommand("트렁크",function(a,b,c)
	if not IsPedSittingInAnyVehicle(plyPed) then
		ESX.ShowNotification(_U('no_vehicle'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		plyVehicle = GetVehiclePedIsIn(plyPed, false)
		if item == vehTrunkItem then
			if not personalmenu.trunkDoorOpen then
				personalmenu.trunkDoorOpen = true
				SetVehicleDoorOpen(plyVehicle, 5, false, false)
			elseif personalmenu.trunkDoorOpen then
...
RegisterCommand("운전석문",function(a,b,c)
	if not IsPedSittingInAnyVehicle(plyPed) then
		ESX.ShowNotification(_U('no_vehicle'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		plyVehicle = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("보조석문",function(a,b,c)
	if not IsPedSittingInAnyVehicle(plyPed) then
		ESX.ShowNotification(_U('no_vehicle'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		plyVehicle = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("왼쪽뒷좌석문",function(a,b,c)
	if not IsPedSittingInAnyVehicle(plyPed) then
		ESX.ShowNotification(_U('no_vehicle'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		plyVehicle = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("오른쪽뒷좌석문",function(a,b,c)
	if not IsPedSittingInAnyVehicle(plyPed) then
		ESX.ShowNotification(_U('no_vehicle'))
	elseif IsPedSittingInAnyVehicle(plyPed) then
		plyVehicle = GetVehiclePedIsIn(plyPed, false)
